...
using CQBot.Models.CQRequest;
using CQBot.Models.CQResponse;
using CQBot.Models.CQResponse.Api;
using System.Web;
...
namespace CQBot.Assist.Http
{
    ...
    {
        public static string ApiUrl { get; set; } = @"http://localhost:5700";
        ...
        public SendPrivateMsgResponse SendPrivateMessage(string id, string message)
        {
            ...
            _Logger.DefaultLine("Sent request.");

            if (response != null)
            {
                json_string = WebRequestHelper.GetResponseString(response);
                _Logger.DefaultLine("Received response.");
            }
